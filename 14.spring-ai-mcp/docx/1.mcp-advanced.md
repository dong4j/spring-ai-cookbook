# MCP 进阶使用

本文档将深入讲解 MCP 的高级功能和使用技巧，帮助您充分利用 MCP 的强大能力。

## 1. Tools（工具）

Tools 是 MCP 服务器暴露给客户端和 LLM 的可执行功能。通过 Tools，LLM 可以调用外部系统来完成各种任务。

### 1.1 工具提供器（ToolCallbackProvider）

Spring AI MCP 提供了多种工具提供器来注册工具：

#### MethodToolCallbackProvider

最常用的工具提供器，通过 `@Tool` 注解自动识别方法并注册为工具：

```java
@Configuration
public class McpServerAutoConfiguration {
    
    @Bean
    public ToolCallbackProvider weatherTools(WeatherService weatherService) {
        return MethodToolCallbackProvider.builder()
                .toolObjects(weatherService)
                .build();
    }
}
```

#### StaticToolCallbackProvider

用于注册静态函数工具，内部使用 `FunctionCallback` 进行包装：

```java
@Bean
public ToolCallbackProvider staticTools() {
    return StaticToolCallbackProvider.builder()
            .functions(List.of(
                new FunctionCallback("calculate", "计算器", 
                    (input) -> "计算结果: " + input)
            ))
            .build();
}
```

#### SyncMcpToolCallbackProvider / AsyncMcpToolCallbackProvider

用于同步和异步的 MCP 工具回调：

```java
@Bean
public ToolCallbackProvider syncMcpTools() {
    return SyncMcpToolCallbackProvider.builder()
            .toolSpecifications(List.of(
                new McpSchema.Tool("syncTool", "同步工具", null, null)
            ))
            .toolHandler((exchange, request) -> {
                // 处理工具调用
                return new McpSchema.CallToolResult(
                    List.of(new McpSchema.TextContent("执行结果"))
                );
            })
            .build();
}
```

### 1.2 工具回调（ToolCallback）

所有工具提供器最终都会构建 `ToolCallback` 接口类型的实例：

- **MethodToolCallback**：普通 `@Tool` 标识的方法工具回调
- **FunctionToolCallback**：函数回调工具
- **SyncMcpToolCallback / AsyncMcpToolCallback**：同步/异步工具回调

### 1.3 使用 ToolContext

在工具方法中，可以通过 `ToolContext` 参数访问 MCP 的上下文信息：

```java
@Service
public class WeatherService {
    
    @Tool(description = "天气查询")
    public String queryWeather(String city, ToolContext toolContext) {
        // 通过 ToolContext 访问 MCP Exchange
        McpToolUtils.getMcpExchange(toolContext)
            .ifPresent(exchange -> {
                // 可以访问客户端能力、调用其他资源等
                System.out.println("客户端能力: " + exchange.getClientCapabilities());
            });
        
        return city + "的天气是18°";
    }
}
```

## 2. Resources（资源）

Resources 是服务器暴露的数据和内容，供客户端读取并作为 LLM 上下文。可以理解为类似 Spring MVC 的接口，用于查询数据。

### 2.1 定义资源

```java
@Bean
public List<McpServerFeatures.SyncResourceSpecification> resource() {
    // 定义资源注解（可选）
    McpSchema.Annotations annotations = 
        new McpSchema.Annotations(List.of(McpSchema.Role.USER), 0.1);
    
    // 创建资源定义
    var databaseResource = new McpSchema.Resource(
        "/v1/api/query",           // URI
        "queryDatabase",           // 名称
        "数据库查询",              // 描述
        "",                        // MIME 类型
        annotations                // 注解
    );
    
    // 定义资源处理器
    var resourceSpecification = 
        new McpServerFeatures.SyncResourceSpecification(
            databaseResource,
            (exchange, request) -> {
                try {
                    // 查询数据
                    var data = Map.of("order", "订单信息");
                    String jsonContent = new ObjectMapper()
                        .writeValueAsString(data);
                    
                    // 返回资源内容
                    return new McpSchema.ReadResourceResult(
                        List.of(new McpSchema.TextResourceContents(
                            request.uri(), 
                            "application/json", 
                            jsonContent
                        ))
                    );
                } catch (Exception e) {
                    throw new RuntimeException("查询失败", e);
                }
            }
        );
    
    return List.of(resourceSpecification);
}
```

### 2.2 客户端读取资源

```java
@Bean
public CommandLineRunner readResources(List<McpSyncClient> mcpSyncClients) {
    return args -> {
        mcpSyncClients.forEach(mcpSyncClient -> {
            // 列出所有可用资源
            List<String> resources = mcpSyncClient.listResources()
                .resources()
                .stream()
                .map(McpSchema.Resource::name)
                .toList();
            System.out.println("可用资源: " + resources);
            
            // 读取特定资源
            McpSchema.ReadResourceRequest request = 
                new McpSchema.ReadResourceRequest("/v1/api/query");
            McpSchema.ReadResourceResult result = 
                mcpSyncClient.readResource(request);
            
            result.contents().forEach(content -> {
                if (content instanceof McpSchema.TextResourceContents textContent) {
                    System.out.println("资源内容: " + textContent.text());
                }
            });
        });
    };
}
```

## 3. Prompts（提示词）

Prompts 允许服务器定义可复用的提示模板，引导 LLM 交互。这对于标准化提示词、提高复用性非常有用。

### 3.1 定义提示词

```java
@Bean
public List<McpServerFeatures.SyncPromptSpecification> prompts() {
    // 创建提示词定义，包含参数
    var prompt = new McpSchema.Prompt(
        "greeting",                                    // 提示词名称
        "一个友好的提示词模板",                        // 描述
        List.of(new McpSchema.PromptArgument(         // 参数列表
            "name",                                    // 参数名
            "名称",                                    // 参数描述
            true                                       // 是否必需
        ))
    );
    
    // 定义提示词处理器
    var promptSpecification = 
        new McpServerFeatures.SyncPromptSpecification(
            prompt,
            (exchange, getPromptRequest) -> {
                // 获取参数
                String nameArgument = (String) 
                    getPromptRequest.arguments().get("name");
                if (nameArgument == null) {
                    nameArgument = "朋友";
                }
                
                // 构建提示词消息
                var userMessage = new McpSchema.PromptMessage(
                    McpSchema.Role.USER,
                    new McpSchema.TextContent(
                        "你好 " + nameArgument + "! 我可以帮助你什么?"
                    )
                );
                
                return new McpSchema.GetPromptResult(
                    "个性化问候语", 
                    List.of(userMessage)
                );
            }
        );
    
    return List.of(promptSpecification);
}
```

### 3.2 客户端获取提示词

```java
@Bean
public CommandLineRunner getPrompts(List<McpSyncClient> mcpSyncClients) {
    return args -> {
        mcpSyncClients.forEach(mcpSyncClient -> {
            // 列出所有提示词
            List<String> prompts = mcpSyncClient.listPrompts()
                .prompts()
                .stream()
                .map(McpSchema.Prompt::name)
                .toList();
            System.out.println("可用提示词: " + prompts);
            
            // 获取特定提示词
            McpSchema.GetPromptRequest request = 
                new McpSchema.GetPromptRequest(
                    "greeting", 
                    Map.of("name", "张三")
                );
            McpSchema.GetPromptResult result = 
                mcpSyncClient.getPrompt(request);
            
            System.out.println("提示词描述: " + result.description());
            result.messages().forEach(message -> {
                System.out.println("消息: " + message.content());
            });
        });
    };
}
```

## 4. Roots（根资源）

Roots 允许客户端告诉服务器应该关注哪些资源范围。当客户端连接到服务器时，会声明应该关注哪些根资源。根资源可以是文件路径，也可以是 HTTP URL 地址。

### 4.1 客户端配置 Roots

首先需要在客户端启用 Roots 能力：

```java
@Component
public static class CustomMcpSyncClientCustomizer 
        implements McpSyncClientCustomizer {
    
    @Override
    public void customize(String serverConfigurationName, 
                          McpClient.SyncSpec spec) {
        
        // 启用 Roots 能力
        McpSchema.ClientCapabilities clientCapabilities = 
            McpSchema.ClientCapabilities.builder()
                .experimental(Map.of())
                .roots(true)  // 启用 Roots
                .sampling()
                .build();
        spec.capabilities(clientCapabilities);
    }
}
```

### 4.2 添加根资源

```java
@Bean
public CommandLineRunner addRoots(List<McpSyncClient> mcpSyncClients) {
    return args -> {
        mcpSyncClients.forEach(mcpSyncClient -> {
            // 添加文件系统根资源
            McpSchema.Root fileRoot = 
                new McpSchema.Root("file:///home/user/projects", "项目目录");
            mcpSyncClient.addRoot(fileRoot);
            
            // 添加 HTTP 根资源
            McpSchema.Root httpRoot = 
                new McpSchema.Root("https://api.example.com/v1", "API端点");
            mcpSyncClient.addRoot(httpRoot);
        });
    };
}
```

### 4.3 服务端处理 Roots 变化

```java
@Bean
public BiConsumer<McpSyncServerExchange, List<McpSchema.Root>> 
        rootsChangeHandler() {
    return (exchange, roots) -> {
        roots.forEach(root -> {
            String uri = root.uri();
            String description = root.description();
            System.out.println("Root资源变化: " + uri + " - " + description);
            
            // 可以在这里监听资源变化，例如文件系统监控
            // 当资源发生变化时，通过日志通知客户端
            McpSchema.LoggingMessageNotification notification = 
                McpSchema.LoggingMessageNotification.builder()
                    .data("资源发生了变化: " + uri)
                    .level(McpSchema.LoggingLevel.INFO)
                    .logger("rootsChangeHandler")
                    .build();
            exchange.loggingNotification(notification);
        });
    };
}
```

## 5. Sampling（采样）

Sampling 允许服务器在执行工具前后通过客户端向 LLM 发起完成请求，实现复杂的智能行为。这类似于回调函数，可以在工具执行过程中与 LLM 进行交互。

### 5.1 客户端配置 Sampling

```java
@Component
public static class CustomMcpSyncClientCustomizer 
        implements McpSyncClientCustomizer {
    
    @Override
    public void customize(String serverConfigurationName, 
                          McpClient.SyncSpec spec) {
        
        // 启用 Sampling 能力
        McpSchema.ClientCapabilities clientCapabilities = 
            McpSchema.ClientCapabilities.builder()
                .experimental(Map.of())
                .roots(true)
                .sampling()  // 启用 Sampling
                .build();
        spec.capabilities(clientCapabilities);
        
        // 设置 Sampling 处理器
        spec.sampling((McpSchema.CreateMessageRequest messageRequest) -> {
            List<McpSchema.SamplingMessage> messages = 
                messageRequest.messages();
            
            McpSchema.CreateMessageResult messageResult = null;
            for (McpSchema.SamplingMessage message : messages) {
                McpSchema.Content content = message.content();
                if ("text".equals(content.type())) {
                    McpSchema.TextContent textContent = 
                        (McpSchema.TextContent) content;
                    System.out.println("收到服务端的回调: " + textContent.text());
                    
                    // 返回确认消息
                    messageResult = McpSchema.CreateMessageResult.builder()
                        .message("我确认：" + textContent.text())
                        .build();
                }
            }
            return messageResult;
        });
    }
}
```

### 5.2 服务端调用 Sampling

在工具方法中，可以通过 `ToolContext` 调用客户端的 Sampling：

```java
@Service
public class WeatherService {
    
    @Tool(description = "天气查询")
    public String queryWeather(String city, ToolContext toolContext) {
        // 在执行查询前，先通过 Sampling 确认
        String confirmation = callMcpSampling(toolContext, city);
        System.out.println("回调函数确认: " + confirmation);
        
        return city + "的天气是18°";
    }
    
    private String callMcpSampling(ToolContext toolContext, String city) {
        StringBuilder stringBuilder = new StringBuilder();
        
        McpToolUtils.getMcpExchange(toolContext)
            .ifPresent(exchange -> {
                // 检查客户端是否支持 Sampling
                if (exchange.getClientCapabilities().sampling() != null) {
                    // 创建 Sampling 请求
                    McpSchema.CreateMessageRequest messageRequest = 
                        McpSchema.CreateMessageRequest.builder()
                            .messages(List.of(
                                new McpSchema.SamplingMessage(
                                    McpSchema.Role.USER,
                                    new McpSchema.TextContent(
                                        "你确定要查询: " + city
                                    )
                                )
                            ))
                            .build();
                    
                    // 调用客户端的 Sampling
                    McpSchema.CreateMessageResult result = 
                        exchange.createMessage(messageRequest);
                    
                    if (result != null && result.content() != null) {
                        stringBuilder.append(result.content().text());
                    }
                }
            });
        
        return stringBuilder.toString();
    }
}
```

## 6. 通讯方式详解

### 6.1 stdio（标准输入输出）

stdio 是最常用的通讯方式，适合本地进程间通信。

#### 服务端配置

```yml
spring:  
  ai:  
    mcp:  
      server:  
        name: stdio-mcp-server  
        version: 1.0.0  
        type: SYNC  
        stdio: true
```

#### 客户端配置

```yml
spring:  
  ai:  
    mcp:  
      client:  
        stdio:  
          connections:  
            server1:  
              command: java  
              args:  
            - -jar  
            - mcp-server.jar
            server2:  
              command: npx  
              args:  
            - -y  
            - "@modelcontextprotocol/server-filesystem"  
            - /Users/user/projects/
```

### 6.2 SSE（Server-Sent Events）

SSE 适合 HTTP 长连接场景，支持服务端主动推送消息。

#### 服务端依赖

```xml
<dependency> 
    <groupId>org.springframework.ai</groupId> 
    <artifactId>spring-ai-starter-mcp-server-webmvc</artifactId> 
</dependency>
```

#### 服务端配置

```yml
spring:  
  ai:  
    mcp:  
      server:  
        name: sse-mcp-server  
        version: 1.0.0  
        sse-endpoint: /sse  
        sse-message-endpoint: /mcp/message
```

#### 客户端配置

```yml
spring:  
  ai:  
    mcp:  
      client:  
        sse:  
          connections:  
            server1: http://localhost:8080
```

### 6.3 WebFlux（响应式）

WebFlux 支持响应式编程，适合高并发异步场景。

#### 服务端依赖

```xml
<dependency> 
    <groupId>org.springframework.ai</groupId> 
    <artifactId>spring-ai-starter-mcp-server-webflux</artifactId> 
</dependency>
```

#### 客户端依赖

```xml
<dependency>  
    <groupId>org.springframework.ai</groupId>  
    <artifactId>spring-ai-starter-mcp-client-webflux</artifactId>  
</dependency>
```

配置方式与 SSE 类似，底层使用响应式编程模型。

## 7. 监听器（Change Consumers）

MCP 提供了多种监听器，用于监听工具、资源、提示词等的变化。

### 7.1 配置监听器

```java
@Component
public static class CustomMcpSyncClientCustomizer 
        implements McpSyncClientCustomizer {
    
    @Override
    public void customize(String serverConfigurationName, 
                          McpClient.SyncSpec spec) {
        
        // 监听工具变化
        spec.toolsChangeConsumer((List<McpSchema.Tool> tools) -> {
            System.out.println("工具列表已更新，共 " + tools.size() + " 个工具");
            tools.forEach(tool -> 
                System.out.println("  - " + tool.name() + ": " + tool.description())
            );
        });
        
        // 监听资源变化
        spec.resourcesChangeConsumer((List<McpSchema.Resource> resources) -> {
            System.out.println("资源列表已更新，共 " + resources.size() + " 个资源");
            resources.forEach(resource -> 
                System.out.println("  - " + resource.name() + ": " + resource.description())
            );
        });
        
        // 监听提示词变化
        spec.promptsChangeConsumer((List<McpSchema.Prompt> prompts) -> {
            System.out.println("提示词列表已更新，共 " + prompts.size() + " 个提示词");
            prompts.forEach(prompt -> 
                System.out.println("  - " + prompt.name() + ": " + prompt.description())
            );
        });
        
        // 监听日志消息
        spec.loggingConsumer((McpSchema.LoggingMessageNotification log) -> {
            System.out.println("[" + log.level() + "] " + 
                log.logger() + ": " + log.data());
        });
    }
}
```

### 7.2 自定义超时配置

```java
@Component
public static class CustomMcpSyncClientCustomizer 
        implements McpSyncClientCustomizer {
    
    @Override
    public void customize(String serverConfigurationName, 
                          McpClient.SyncSpec spec) {
        
        // 设置请求超时时间
        spec.requestTimeout(Duration.ofSeconds(30));
    }
}
```

## 8. 最佳实践

### 8.1 工具设计原则

1. **单一职责**：每个工具应该只做一件事
2. **清晰的描述**：工具描述应该清晰说明功能和参数
3. **错误处理**：工具应该妥善处理异常情况
4. **幂等性**：工具调用应该是幂等的，重复调用不应该产生副作用

### 8.2 资源设计原则

1. **RESTful 风格**：资源 URI 应该遵循 RESTful 设计原则
2. **缓存策略**：考虑资源的缓存需求
3. **数据格式**：使用标准的数据格式（JSON、XML 等）

### 8.3 性能优化

1. **异步处理**：对于耗时操作，使用异步工具回调
2. **批量操作**：尽量支持批量操作，减少网络往返
3. **连接池**：合理配置连接池大小

### 8.4 安全性考虑

1. **输入验证**：始终验证客户端输入
2. **权限控制**：实现适当的权限控制机制
3. **敏感数据**：避免在日志中输出敏感信息

## 9. 总结

本文档详细介绍了 MCP 的高级功能，包括：

- **Tools**：如何定义和使用工具
- **Resources**：如何暴露和读取资源
- **Prompts**：如何定义可复用的提示词
- **Roots**：如何管理根资源
- **Sampling**：如何实现服务端与客户端的交互
- **通讯方式**：stdio、SSE、WebFlux 的配置和使用
- **监听器**：如何监听各种变化事件

通过合理使用这些功能，您可以构建强大而灵活的 MCP 应用。

> 📖 **相关文档**：
> - [MCP 是最优解吗？](2.mcp-not-best.md) - 了解 MCP 的适用场景
> - [MCP 的局限性与安全性](3.mcp-problems.md) - 了解 MCP 的限制和安全注意事项
